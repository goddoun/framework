...
public class SeedCipher
{
    ...
    private void SeedEncrypt(final byte[] pbData, final int[] pdwRoundKey, final byte[] outData) {
        final int[] L0 = { 0 };
        final int[] L2 = { 0 };
        final int[] R0 = { 0 };
        final int[] R2 = { 0 };
        ...
        if (!this.ENDIAN) {
            ...
            this.EndianChange(L2);
            this.EndianChange(R0);
            this.EndianChange(R2);
        }
        K[0] = pdwRoundKey[nCount++];
        K[1] = pdwRoundKey[nCount++];
        this.SeedRound(L0, L2, R0, R2, K);
        K[0] = pdwRoundKey[nCount++];
        K[1] = pdwRoundKey[nCount++];
        this.SeedRound(R0, R2, L0, L2, K);
        K[0] = pdwRoundKey[nCount++];
        K[1] = pdwRoundKey[nCount++];
        this.SeedRound(L0, L2, R0, R2, K);
        K[0] = pdwRoundKey[nCount++];
    ...
    private void SeedDecrypt(final byte[] pbData, final int[] pdwRoundKey, final byte[] outData) {
        final int[] L0 = { 0 };
        final int[] L2 = { 0 };
        final int[] R0 = { 0 };
        final int[] R2 = { 0 };
    ...
    private void EncRoundKeyUpdate0(final int[] K, final int[] A, final int[] B, final int[] C, final int[] D, final int Z) {
        final int T0 = A[0];
        A[0] = (A[0] >>> 8 ^ B[0] << 24);
        B[0] = (B[0] >>> 8 ^ T0 << 24);
        final int T2 = A[0] + C[0] - this.KC[Z];
        final int T3 = B[0] + this.KC[Z] - D[0];
        K[0] = (this.SS0[this.GetB0(T2)] ^ this.SS1[this.GetB1(T2)] ^ this.SS2[this.GetB2(T2)] ^ this.SS3[this.GetB3(T2)]);
        K[1] = (this.SS0[this.GetB0(T3)] ^ this.SS1[this.GetB1(T3)] ^ this.SS2[this.GetB2(T3)] ^ this.SS3[this.GetB3(T3)]);
    }
    
    private void EncRoundKeyUpdate1(final int[] K, final int[] A, final int[] B, final int[] C, final int[] D, final int Z) {
        final int T0 = C[0];
        C[0] = (C[0] << 8 ^ D[0] >>> 24);
        D[0] = (D[0] << 8 ^ T0 >>> 24);
        final int T2 = A[0] + C[0] - this.KC[Z];
    ...
    private void SeedEncRoundKey(final int[] pdwRoundKey, final byte[] pbUserKey) {
        final int[] A = { 0 };
        final int[] B = { 0 };
        final int[] C = { 0 };
        final int[] D = { 0 };
        ...
        final int T0 = A[0] + C[0] - this.KC[0];
        final int T2 = B[0] - D[0] + this.KC[0];
        pdwRoundKey[0] = (this.SS0[this.GetB0(T0)] ^ this.SS1[this.GetB1(T0)] ^ this.SS2[this.GetB2(T0)] ^ this.SS3[this.GetB3(T0)]);
        pdwRoundKey[1] = (this.SS0[this.GetB0(T2)] ^ this.SS1[this.GetB1(T2)] ^ this.SS2[this.GetB2(T2)] ^ this.SS3[this.GetB3(T2)]);
    ...
    public byte[] encrypt(final byte[] sbuffer, final String key) {
        int[] sRoundKey = null;
        try {
            sRoundKey = new int[32];
            this.SeedEncRoundKey(sRoundKey, key.getBytes(DefineSeedElement.DEFAULT_CHARSET));
            final byte[] inDataBuffer = this.padding.addPadding(sbuffer, 16);
            final byte[] encryptBytes = new byte[inDataBuffer.length];
            for (int rt = inDataBuffer.length / 16, j = 0; j < rt; ++j) {
                final byte[] sSource = new byte[16];
                final byte[] sTarget = new byte[16];
                System.arraycopy(inDataBuffer, j * 16, sSource, 0, 16);
                this.SeedEncrypt(sSource, sRoundKey, sTarget);
    ...
    public byte[] encrypt(final byte[] sbuffer, final byte[] szKey) {
        final int[] sRoundKey = new int[32];
        this.SeedEncRoundKey(sRoundKey, szKey);
        final byte[] inDataBuffer = this.padding.addPadding(sbuffer, 16);
        final byte[] encryptBytes = new byte[inDataBuffer.length];
        for (int rt = inDataBuffer.length / 16, j = 0; j < rt; ++j) {
            final byte[] sSource = new byte[16];
    ...
    public byte[] encrypt(final String inData, final byte[] szKey) {
        return this.encrypt(inData.getBytes(), szKey);
    }
    
    public byte[] encrypt(final String inData, final byte[] szKey, final String charset) throws UnsupportedEncodingException {
        return this.encrypt(inData.getBytes(charset), szKey);
    }
    
    public byte[] decrypt(final byte[] encryptBytes, final String key) {
        byte[] szKey = null;
        try {
            szKey = key.getBytes();
            return this.decrypt(encryptBytes, szKey);
        }
        catch (Exception e) {
            Logger.error(e);
            return null;
    ...
    public byte[] decrypt(final byte[] encryptBytes, final byte[] szKey) {
        int[] sRoundKey = null;
        byte[] decryptBytes = null;
        int rt = -1;
        try {
            sRoundKey = new int[32];
            decryptBytes = new byte[encryptBytes.length];
            this.SeedEncRoundKey(sRoundKey, szKey);
            rt = encryptBytes.length / 16;
            final byte[] sSource = new byte[16];
            final byte[] sTarget = new byte[16];
            for (int j = 0; j < rt; ++j) {
                System.arraycopy(encryptBytes, j * 16, sSource, 0, 16);
                this.SeedDecrypt(sSource, sRoundKey, sTarget);
                System.arraycopy(sTarget, 0, decryptBytes, j * 16, 16);
            }
            return this.padding.removePadding(decryptBytes, 16);
        }
        catch (Exception e) {
            Logger.error(e);
            return null;
    ...
    public String decryptAsString(final byte[] encryptBytes, final byte[] szKey) {
        return new String(this.decrypt(encryptBytes, szKey));
    }
    
    public String decryptAsString(final byte[] encryptBytes, final byte[] szKey, final String charset) throws UnsupportedEncodingException {
        return new String(this.decrypt(encryptBytes, szKey), charset);
    }
}
